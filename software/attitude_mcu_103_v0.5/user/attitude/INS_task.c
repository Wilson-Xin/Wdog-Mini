...
#define IMU_temp_PWM(pwm)   __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_3, pwm)//pwm给定
...
void get_angle(fp32 q[4], fp32 *yaw, fp32 *pitch, fp32 *roll)//转换角度
{
...
{	
	...
	tempx=(int16_t)(temp+0.5f);//四舍五入
	
	txbuf[0]=0x01;//命令字
	txbuf[1]=(uint8_t)(yaw/256);//yaw高
	txbuf[2]=(uint8_t)(yaw%256);//yaw低
	txbuf[3]=(uint8_t)(pitch/256);//pitch高
	txbuf[4]=(uint8_t)(pitch%256);//pitch低
	txbuf[5]=(uint8_t)(roll/256);//roll高
	txbuf[6]=(uint8_t)(roll%256);//roll低
	txbuf[7]=tempx;//温度
...
void DMA1_Channel2_IRQHandler(void)
{
  ...
  /* USER CODE BEGIN DMA1_Channel2_IRQn 1 */
		 ...
		 if(__HAL_DMA_GET_FLAG(hspi1.hdmarx,__HAL_DMA_GET_TC_FLAG_INDEX(hspi1.hdmarx))!=RESET)//等待DMA1_Steam0传输完成
			{ 	
				__HAL_DMA_CLEAR_FLAG(hspi1.hdmarx,__HAL_DMA_GET_TC_FLAG_INDEX(hspi1.hdmarx));//清除DMA1_Steam0传输完成标志 

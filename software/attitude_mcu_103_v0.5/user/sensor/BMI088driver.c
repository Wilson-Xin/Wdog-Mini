...
void ist8310_acc_cal(uint8_t acc[6]);

int16_t acc_offset[3]={0,0,0};
int16_t gyro_offset[3]={3,3,3};
...
void BMI088_accel_read_over(uint8_t *rx_buf, fp32 accel[3], fp32 *time)
{
    ...
    uint32_t sensor_time;
		...
		//ist8310_acc_cal(rx_buf);
	
    bmi088_raw_temp = (int16_t)((rx_buf[1]) << 8) | rx_buf[0];
    accel[0] = BMI088_ACCEL_SEN * (bmi088_raw_temp-acc_offset[0]);
    bmi088_raw_temp = (int16_t)((rx_buf[3]) << 8) | rx_buf[2];
    accel[1] = BMI088_ACCEL_SEN * (bmi088_raw_temp-acc_offset[1]);
    bmi088_raw_temp = (int16_t)((rx_buf[5]) << 8) | rx_buf[4];
    accel[2] = BMI088_ACCEL_SEN * (bmi088_raw_temp-acc_offset[2]);
...
void BMI088_gyro_read_over(uint8_t *rx_buf, fp32 gyro[3])
{
    int16_t bmi088_raw_temp;
    bmi088_raw_temp = (int16_t)((rx_buf[1]) << 8) | rx_buf[0];
    gyro[0] = BMI088_GYRO_SEN * (bmi088_raw_temp-gyro_offset[0]);
    bmi088_raw_temp = (int16_t)((rx_buf[3]) << 8) | rx_buf[2];
    gyro[1] = BMI088_GYRO_SEN * (bmi088_raw_temp-gyro_offset[1]);
    bmi088_raw_temp = (int16_t)((rx_buf[5]) << 8) | rx_buf[4];
    gyro[2] = BMI088_GYRO_SEN * (bmi088_raw_temp-gyro_offset[2]);
}

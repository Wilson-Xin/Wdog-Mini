...
void MX_CAN_Init(void)
{
 ...
 sFilterConfig.FilterIdHigh = (0x01<<5); //设置过滤器ID高16位---0x01作验证码
 sFilterConfig.FilterIdLow = 0;//设置过滤器ID低16位
 sFilterConfig.FilterMaskIdHigh = (0xff<<5);//设置过滤器掩码高16位
 sFilterConfig.FilterMaskIdLow = 0x0|0x02;//设置过滤器掩码低16位--只接收数据帧
...
void CAN_senddata(CAN_HandleTypeDef *hcan,uint8_t *txbuf)
{
   TXHeader.StdId=0x12;
	 TXHeader.ExtId=0x00;
	 TXHeader.DLC=8;
	 ...
	 HAL_CAN_AddTxMessage(hcan,&TXHeader,txbuf,(uint32_t*)CAN_TX_MAILBOX0);
}
